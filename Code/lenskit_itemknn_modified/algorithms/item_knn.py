...
class ItemItem(Predictor):
    ...
    def _compute_similarities(self, rmat):
        ...
        dense_tensor = smat.to_scipy().todense()
        ...
        for row in range(dense_tensor.shape[1]):
            row_values = dense_tensor[row, :]
            row_values_tensor = torch.tensor(row_values.tolist())
            top_values, top_indices = torch.topk(row_values_tensor, k=20)
        ...
        modified_csr_matrix = CSR.from_scipy(matrix, False)

        return modified_csr_matrix
